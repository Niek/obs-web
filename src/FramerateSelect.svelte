<script>
  ...
  onMount(async function () {
    ...
    const matchingFps = framerateData.find(f => f.fpsNumerator === data.fpsNumerator);
    currentFramerate = matchingFps.fps;
  })

  async function SetFramerate(event) {
    console.log('SetFramerate', event.target.value, 'FPS')
...
<div class="select" style="margin: 0 .5rem .5rem 0;">
  <select bind:value={currentFramerate} title="Change FPS" on:change={SetFramerate}>
